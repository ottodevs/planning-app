...
import vaultAbi from '@aragon/apps-vault/build/contracts/Vault.json'
...
const getRepoData = repo => {
  try {
    const data = client.request(repoData(repo))
    return data
  } catch (error) {
    // console.error('getRepoData failed:', error)
  }
}
...
async function handleEvents(response) {
  ...
  const repoIndex = appState.repos.findIndex(
    repo => repo.id === response.returnValues.repoId
  )
...
async function syncRepos(state, { repoId /*, ...eventArgs*/ }) {
  ...
  try {
    const updatedState = await updateState(state, repoId, transform)
    return updatedState
  } catch (error) {
    // console.error('updateState failed to return:', error)
  }
}
...
function syncIssues(state, { issueNumber /*, ...eventArgs*/ }, data) {
  ...
  try {
    const updatedState = updateIssueState(state, issueNumber, data)
    return updatedState
  } catch (error) {
    // console.error('updateIssueState failed to return:', error)
  }
}
...
async function syncSettings(state) {
  try {
    ...
    // console.error('[Projects script] syncSettings settings failed:', error)
  }
}
...
async function updateIssueDetail(data, response) {
  ...
  if (status.indexOf(data.workStatus) >= SUBMISSION_STAGE) {
    const submissionData = await loadSubmissionData(response.returnValues)
...
function loadRepoData(id) {
  return new Promise(resolve => {
    app.call('getRepo', id).subscribe(({ owner, index }) => {
      ...
      return void getRepoData(_repo).then(({ node }) => {
        ...
        return void resolve({ _repo, _owner, index, metadata })
      })
    })
  })
}
...
async function updateState(state, id, transform) {
  ...
  const { repos = [] } = state
  try {
    const newRepos = await checkReposLoaded(repos, id, transform)
    const newState = { ...state, repos: newRepos }
    return newState
  } catch (error) {
    // console.error(
...
function updateIssueState(state, issueNumber, data) {
  ...
  const { issues = [] } = state
  try {
    const newIssues = checkIssuesLoaded(issues, issueNumber, data)
    const newState = { ...state, issues: newIssues }
    return newState
  } catch (error) {
    // console.error(
