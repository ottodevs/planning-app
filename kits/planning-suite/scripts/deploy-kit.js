...
module.exports = async (
  ...
  let arappFileName
  console.log('terminamos', returnKit, network)
  ...
  console.log('after returnKit', ensAddress, arappFileName)
  ...
  console.log('dao faco', daoFactoryAddress)
  ...
  if (daoFactoryAddress) {
    ...
    daoFactory = (await deployDAOFactory(null, { artifacts, verbose: false }))
      .daoFactory
  }
  ...
  console.log('continua after minimefactor', kitContractName)

  const aragonid = await ens.owner(namehash('aragonid.eth'))
  const kit = await artifacts
    .require(kitContractName)
    .new(daoFactory.address, ens.address, minimeFac.address, aragonid, appIds)

  console.log('llegamos al kit', kit, returnKit)
